...
define(function(require, exports){
	...
	var VMCtrl,
		...
		MVVM = null,
		util = require('util'),
		// avalon = require('@libs/avalon/avalon.shim.min.js');
		// avalon = require('@libs/avalon/avalon.min.js');
		avalon = require('@libs/avalon/avalon1.3.1.js');
	...
	function defineVMCtrl() {
		VMCtrl = pubjs.Class.extend({
			...
			/**
			 * avalon的$model属性，用于去掉vm绑定的多余属性
			 */
			model: function(obj) {
				if (obj && obj.$model && obj.$watch) {
					return this._killAvalonHas(obj.$model);
				}
				return obj;
			},
			/**
			 * avalon获取的数据在某些时候会挂载hasOwnProperty函数，在getData或getItem的时候干掉
			 */
			_killAvalonHas: function(obj) {
				if (util.isPlainObject(obj)) {
					if (util.has(obj, 'hasOwnProperty')) {
			...
			getItem: function(uri) {
				return this._killAvalonHas(util.prop(this.$.$model, uri));
			},
			/**
			 * 获取vm中的数据
			...
			getData: function(key) {
				...
				return this._killAvalonHas(data);
			},
			/**
			 * 监听VM中的字段
			...
			watch: function(uri, func, context) {
				...
				if (parent && util.isFunc(parent.$watch)) {
					parent.$watch(name, function(newV, oldV) {
						func.call(context, newV, oldV, uri);
					});
				} else {
